...
class SwitchingDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # -------movie recommendation feedback-----------
        # self.movie_tohidden = nn.Linear(in_features=n_movies, out_features=hidden_size)
        # self.language_out = nn.Linear(in_features=hidden_size, out_features=vocab_size)
        #------------------------------------------------
    ...
    def forward(self, input, lengths, context, movie_recommendations, log_probabilities, sample_movies,
        ...
        # -------movie recommendation feedback-----------
        # movie_recommendations_hidden = self.movie_tohidden(movie_recommendations) #(batch_size, n_movie) to (batch_size, hidden)
        # movie_recommendations_hidden = F.relu(movie_recommendations_hidden) #apply relu
        # movie_rec_bias = self.language_out(movie_recommendations_hidden)
        # movie_rec_bias_fit = torch.reshape(movie_rec_bias, (batch_size, 1,self.vocab_size)) #(batch_size, voc) to (batch_size, 1, voc)
        # movie_rec_bias_fit = movie_rec_bias_fit.expand(batch_size,hidden.shape[1],self.vocab_size) #(batch_size, 1, voc) to (batch_size, seq_len, voc)
        # language_output = language_output + movie_rec_bias_fit # (Wo+b)+bm
        # -------------------------------------------------
        ...
        #print("switching decoder:{}".format(output.size()))
        return output
