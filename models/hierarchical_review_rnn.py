...
class RHRNN(nn.Module):
    ...
    Input: Input["reviews"] (batch, max_conv_length, max_utterance_length) Long Tensor
    """
    ...
    def __init__(self,
        ...
        # Otherwise instantiate a new gensen module
            ...
            ####設定GRU維度，考慮use movie occurrences的設定
        self.sentence_encoder = nn.GRU(
            input_size=2048  if self.use_gensen
        ...
        self.conversation_encoder = nn.GRU(
            # input_size=(2 * self.params['sentence_encoder_hidden_size'])+ 1 + (self.params['use_movie_occurrences'] == "sentence"),
            input_size=(2 * self.params['sentence_encoder_hidden_size'])+  (self.params['use_movie_occurrences'] == "sentence"),
    ...
    def get_sentence_representations(self, dialogue, lengths):
    ...
    def forward(self, input_dict, return_all=True, return_sentence_representations=False):
        # if self.params['use_movie_occurrences']:
        #     self.movie_occurrences = input_dict["movie_occurrences"] 
        # else:
        #     self.movie_occurrences = None
        # get sentence representations
        sentence_representations = self.get_sentence_representations(input_dict["reviews"], input_dict["lengths"])
