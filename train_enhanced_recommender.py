...
import config
from models.enhanced_recommender_model import Recommender
...
def train(model, batch_loader, nb_epochs, patience, save_path):
    ...
    while epoch < nb_epochs:
        ...
        for i in tqdm(range(n_train_batches)):
            ...
            # print("batch[dialog]:{}".format(batch["dialogue"].size()))
            # print("batch[lengths]:{}".format(len(batch["lengths"])))
            # print("batch[target]:{}".format(batch["target"].size()))
            # print("batch[movie_occurrences]:{}".format(len(batch["movie_occurrences"])))
...
def explore_params(params_seq, data="movie_dialogue", hred=False):
    ...
    for (save_path, params, train_params) in params_seq:
        ...
        if data == "movie_dialogue_pretrained":
            ...
            batch_loader = DialogueBatchLoader(
                sources=sources,
                batch_size=train_params["batch_size"] #"batch_size": 4
            )
            model = model_class(train_vocab=batch_loader.train_vocabulary, n_movies=batch_loader.n_movies, 
                                movie_rep_matrix=batch_loader.movie_rep_matrix,params=params)
            train(
                model,
...
if __name__ == '__main__':
    params_seq = [(config.ENHANCED_RECOMMENDER_MODEL, test_params.enhanced_recommender_params, test_params.train_enhanced_recommender_params)]
