...
class DialogueBatchLoader(object):
    def __init__(self, sources, batch_size, 
                 ...
                 review_length_limit=config.REVIEW_LENGTH_LIMIT, 
                 review_sentence_length_limit=config.REVIEW_SENTENCE_WORD_LENGTH_LIMIT, 
                 training_size=-1,
                 data_path=config.REDIAL_DATA_PATH,
                 train_path=config.TRAIN_PATH,
                 valid_path=config.VALID_PATH,
                 test_path=config.TEST_PATH,
                 review_data_path=config.REVIEW_DATA_PATH,
                 movie_rep_matrix_path=config.MOVIE_REP_MATRIX_PATH,
                 review_knowledge_train=config.REDIAL_REVIEW_KNOWLEDGE_TRAIN,
                 review_knowledge_valid=config.REDIAL_REVIEW_KNOWLEDGE_VALID,
                 review_knowledge_test=config.REDIAL_REVIEW_KNOWLEDGE_TEST,
                 rwr_data_path=config.RWR_DATA_PATH,
                 rwr_train=config.RWR_TRAIN,
                 rwr_test=config.RWR_TEST,
                 rwr_valid=config.RWR_VALID,
        ...
        self.review_length_limit = review_length_limit
        self.review_sentence_length_limit = review_sentence_length_limit
        ...
        self.review_data_path = {"train": os.path.join(review_data_path, review_knowledge_train),
                          "valid": os.path.join(review_data_path, review_knowledge_valid),
                          "test": os.path.join(review_data_path, review_knowledge_test)}
        self.rwr_data_path =  {"train": os.path.join(rwr_data_path, rwr_train),
                          "valid": os.path.join(rwr_data_path, rwr_valid),
                          "test": os.path.join(rwr_data_path, rwr_test)}
    ...
    def _get_dataset_characteristics(self):
        ...
        if "sentiment_analysis" in self.sources:
            ...
            # (self.db2id[int(key)], self.db2name[int(key)], answers, i)
        ...
        if "ratings_seq" in self.sources:
            self.ratings_seq_data = {key: load_pickle_data(val) for key, val in self.rwr_data_path.items()}

        if "RW_rec" in self.sources:
            self.re_rec_data = {key: load_pickle_data(val) for key, val in self.rwr_data_path.items()}
        
        if "review_rep" in self.sources:
            self.review_knowledge_data = {key: load_pickle_data(val) for key, val in self.review_data_path.items()}
        ...
        elif "review_rep" in self.sources:
            data = self.review_knowledge_data
        elif "RW_rec" in self.sources:
            data = self.re_rec_data
        elif "ratings_seq" in self.sources:
            data = self.ratings_seq_data
    ...
    def extract_form_data(self, data):
        ...
        for (i, conversation) in enumerate(data):
            ...
            # get movies that are in both forms. Do not take empty movie names 取得redial有的電影
            for key in gen:
                ...
                #key =>電影id, init_q跟resp_q存每個電影id對應的suggest, seen, liked
        return form_data
    
    # def extract_rw_rec_form_data(self, data):
    #     #取得re_rec資料
    #     """
    #     get form data from data. For sentiment analysis.
    #     :param data:
    ...
    #         for key in gen:
    #             #print(key)
    #             if ((init_q[key]["seen"] == 1) & (init_q[key]["liked"] == 1)):
    #                 likedmovielist.append(self.db2id[int(key)])
    #         #build target 
    ...
    #                        resp_q[key]["seen"],
    #                        resp_q[key]["liked"]]
    #             form_data.append((self.db2id[int(key)], self.db2name[int(key)], answers, i, target_list))
    #             #key =>電影id, init_q跟resp_q存每個電影id對應的suggest, seen, liked
    ...
    def extract_review(self, conversation, flatten_messages=True):
        """
        :param conversation: conversation dictionary. keys : 'conversationId', 'respondentQuestions', 'messages',
         'movieMentions', 'respondentWorkerId', 'initiatorWorkerId', 'initiatorQuestions'
         :param flatten_messages
        ...
        for message in conversation:
            ...
            text = tokenize_review(message)
        text = self.truncate_review(text)
    ...
    def truncate_review(self, dialogue):
        # truncate conversations that have too many utterances
        if len(dialogue) > self.review_length_limit:
            dialogue = dialogue[:self.review_length_limit]
            # target = target[:self.review_length_limit]
            # senders = senders[:self.review_length_limit]
            # if "movie_occurrences" in self.sources:
            #     movie_occurrences = {
            #         key: val[:self.review_length_limit] for key, val in movie_occurrences.items()
            #     }
        # truncate utterances that are too long
        for (i, utterance) in enumerate(dialogue):
            if len(utterance) > self.review_sentence_length_limit:
                dialogue[i] = dialogue[i][:self.review_sentence_length_limit]
                # target[i] = target[i][:self.review_length_limit]
                # if "movie_occurrences" in self.sources:
                #     for movieId, value in movie_occurrences.items():
                #         value[i] = value[i][:self.review_length_limit]
        return dialogue
    ...
    def _load_dialogue_batch(self, subset, flatten_messages):
        batch = {"senders": [], "dialogue": [], "lengths": [], "target": [], "seeker":[], "recommender":[]}
        if "movie_occurrences" in self.sources:
        ...
        for i, conversation in enumerate(batch_data):
            ...
            batch["seeker"]=conversation['initiatorQuestions']
            batch["recommender"]=conversation['respondentQuestions']
            if "movie_occurrences" in self.sources:
        ...
        batch["target"] = Variable(torch.LongTensor(
            self.dtext_to_ids(batch["target"], max_utterance_len, max_conv_len)))
    ...
    def _load_sentiment_analysis_batch(self, subset, flatten_messages=True, cut_dialogues=-1):
        ...
        # get batch
        batch_data = self.form_data[subset][self.batch_index[subset] * self.batch_size: #batch_size=16
                                            (self.batch_index[subset] + 1) * self.batch_size]

        for i, example in enumerate(batch_data):
            # (movieId, movieName, answers, conversationIndex) = example
            # (5421, 'Starship Troopers ', [1, 0, 1, 1, 0, 1], 0)
            # 1
            # (2556, 'Spaceballs ', [1, 1, 1, 0, 1, 1], 0)
            # 2
            # (1345, 'The Wrestler ', [1, 0, 0, 1, 0, 0], 0)
            # 3
            # (5062, 'Stardust ', [0, 1, 1, 0, 1, 1], 0)
        ...
        batch["dialogue"] = Variable(torch.LongTensor(
            self.dtext_to_ids(batch["dialogue"], max_utterance_len, max_conv_len)))  # (batch, conv_len, utt_len)
        batch["senders"] = Variable(torch.FloatTensor(
            [senders + [0] * (max_conv_len - len(senders)) for senders in batch["senders"]]))
        ...
        print("batch[movieIds]:{}".format(len(batch["movieIds"])))
        print("batch[lengths]:{}".format(len(["lengths"])))
        print("batch[dialogue]:{}".format(batch["dialogue"].size()))
        print("batch[senders]:{}".format(batch["senders"].size()))
        print("batch[conversation_lengths]:{}".format(len(batch["conversation_lengths"])))
        print("batch[movie_occurrences]:{}".format(batch["movie_occurrences"].size()))
    ...
    def _load_rw_rec_batch(self, subset, flatten_messages=True, cut_dialogues=-1):
        ...
        ##batch["input_dist"]= Variable(torch.FloatTensor([item.cpu().detach().numpy() for item in batch["input_dist"]])).cuda() 

        #train seq autoencoder
        #return {"input": Variable(torch.from_numpy(batch["input_dist"]).float()),
        #        "target": Variable(torch.from_numpy(batch["target"]).float())}
    ...
    def _load_movie_review_ae_batch(self, subset, flatten_messages=True, cut_dialogues=-1): #, batch_input, max_num_inputs=None):
        batch = {"convId": [], "movieName": [], "reviews": [], "lengths": [], "target": []}

        # get batch
        batch_data = self.review_knowledge_data[subset][self.batch_index[subset] * self.batch_size:
                                               (self.batch_index[subset] + 1) * self.batch_size]
        #print(batch_data)
        for i, example in enumerate(batch_data):
            if self.process_at_instanciation:
                dialogue, target, senders, movie_occurrences = example
            else:
                dialogue = self.extract_review(example['reviews'],flatten_messages=flatten_messages)
            ...
            for rev in dialogue:
                sum_r  = sum_r+len(rev)
            if ((sum_r == 5) | (sum_r == 10) | (sum_r==0)):
                batch["lengths"].append([50 for rev in range(10)])
            else:
                batch["lengths"].append([len(rev) for rev in dialogue])
            batch["reviews"].append(dialogue)
            for item in example['target']:
                batch["target"].append(item)

        max_utterance_len = max([max(x) for x in batch["lengths"]])
        max_conv_len = len(dialogue)
        batch["conversation_lengths"] = np.array([len(x) for x in batch["lengths"]])
        # replace text with ids and pad sentences
        batch["lengths"] = np.array(
            [lengths + [0] * (max_conv_len - len(lengths)) for lengths in batch["lengths"]]
        )
        batch["reviews"] = Variable(torch.LongTensor(
            self.text_to_ids(batch["reviews"],max_utterance_len, max_conv_len)))  # (batch, conv_len, utt_len)
        batch["target"] = Variable(torch.LongTensor(batch["target"]))  # (batch, 6)
    ...
    def load_batch(self, subset="train",
        ...
        if "dialogue" in self.sources:
            ...
            batch = self._load_movie_review_ae_batch(subset, flatten_messages, cut_dialogues=cut_dialogues)
        ...
        # elif "review_ae" in self.sources:
        #     batch = self._load_movie_review_ae_batch(subset, flatten_messages, cut_dialogues=cut_dialogues)
           # print("load_batch -> review: {}".format(batch))
            #,batch_input=batch_input, max_num_inputs=max_num_inputs)
            # print(batch)
            # print("batch[convId]:{}".format(len(batch["convId"])))
            # print("batch[reviews]:{}".format(batch["reviews"].size()))
            # print("batch[lengths]:{}".format(len(batch["lengths"])))
            # print("batch[target]:{}".format(batch["target"].size()))

        self.batch_index[subset] = (self.batch_index[subset] + 1) % self.n_batches[subset]

        return batch

    def check_review(self, utt):
        label = 1
        if((utt==['n']) | (utt ==['o']) | (utt == ['_']) | (utt == ['r']) | (utt == ['e']) | (utt == ['v']) | (utt == ['i']) |
         (utt == ['w']) | (utt == ['s']) & (len(utt) == 1)):
            label = 0
        if(len(utt)==0):
            label = 0
        return label
    ...
    def text_to_ids(self, dialogue, max_utterance_len, max_conv_len):
        ...
        :return: padded dialogue
        """
        dialogue = [[[self.token2id(w) for w in utterance] + [self.word2id['<pad>']] * (max_utterance_len - len(utterance))
         if self.check_review(utterance) else [randrange(0,len(self.word2id)) for i  in range(50)] 
         for utterance in conv] + [[self.word2id['<pad>']] * max_utterance_len] * (max_conv_len - len(conv)) for conv in dialogue]
        #     dialogue = [[[randrange(len(self.word2id)) for i  in range(max_utterance_len)] for utterance in conv] +
        #                 [[self.word2id['<pad>']] * max_utterance_len] * (max_conv_len - len(conv)) for conv in dialogue]
        # else: 
            # dialogue = [[[self.token2id(w) for w in utterance] + [self.word2id['<pad>']] * (max_utterance_len - len(utterance)) for utterance in conv] +
            #             [[self.word2id['<pad>']] * max_utterance_len] * (max_conv_len - len(conv)) for conv in dialogue]
        return dialogue
    
    # def reviewtext_to_ids(self, dialogue,max_utterance_len):
    #     """
    #     replace with corresponding ids.
    #     Pad each utterance to max_utterance_len. And pad each conversation to max_conv_length
    #     :param dialogue: [[[word1, word2, ...]]]
    ...
    #     """
    #     dialogue = [[self.token2id(w) for w in utterance] +
    #                  [self.word2id['<pad>']] * (max_utterance_len - len(utterance)) for utterance in dialogue]
    #     return dialogue
