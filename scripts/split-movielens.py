...
with open(os.path.join(movielens_path, "train_ratings"), 'w') as outfile:
    writer = csv.writer(outfile)
    for dat in data[:int(.8 * n_data)]:
        writer.writerow(dat)
with open(os.path.join(movielens_path, "valid_ratings "), 'w') as outfile:
    writer = csv.writer(outfile)
    for dat in data[int(.8 * n_data):int(.9 * n_data)]:
        writer.writerow(dat)
with open(os.path.join(movielens_path, "test_ratings"), 'w') as outfile:
    writer = csv.writer(outfile)
